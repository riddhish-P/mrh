...
def grad_elec(mc_grad, mo_coeff=None, ci=None, atmlst=None, verbose=None):
    ...
    casdm1 = np.asarray (mc.make_casdm1()) #ci, ncas, nelecas)
    casdm2 = np.asarray (mc.make_casdm2()) #ci, ncas, nelecas)
# gfock = Generalized Fock, Adv. Chem. Phys., 69, 63
    ...
    log.timer('LASSCF nuclear gradients', *time0)
    return de
...
class Gradients(casci_grad.Gradients):
    ...
    def kernel(self, mo_coeff=None, ci=None, atmlst=None, verbose=None):
        ...
        print('----------LASSCF gradients---------')
        for k in range(self.mol.natm):
            print ( '| {} {} , {:.7f} , {:.7f} , {:.7f} |'.format(k, self.mol.atom_pure_symbol(k) , de[k][0], de[k][1], de[k][2] ))
        print('---------------------------------')
